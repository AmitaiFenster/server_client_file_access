...
def conn():
    ...
    try:
        ...
        while status == -1:
            ...
            s.send(username)
            rcv = s.recv(4)
            status = struct.unpack("i", rcv)[0]
            if status == -1:
                print "user dosn't exist, try again"
        status = -1
        while status == -1:
            password = raw_input("\nEnter a password: ")
            s.send(password)
            status = struct.unpack("i", s.recv(4))[0]
            if status == -1:
...
def upld(file_name):
    ...
    try:
        ...
        s.send(struct.pack("h", sys.getsizeof(base_file_name)))
        s.send(base_file_name)
        # Wait for server ok then send file size
        if s.recv(BUFFER_SIZE) == '-1':
            print "File exists on the server but you don't have access"
            content.close()
            return
